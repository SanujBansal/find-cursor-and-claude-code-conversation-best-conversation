...
const navItems = [
  ...
  },
  {
    href: "/rules",
    label: "Project Rules",
    hint: "Cursor / AGENTS / CLAUDE",
    icon: (
      <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.75}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m6.75 12h-6m6 3h-6m0-9H6.375a1.125 1.125 0 00-1.125 1.125v15.75c0 .621.504 1.125 1.125 1.125h11.25c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9H8.25" />
      </svg>
    ),
  },
  {
    href: "/suggestions",
    label: "Improve the Vibe",
    hint: "Prompt upgrade coach",
    icon: (
      <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.75}>
...
export function Sidebar() {
  ...
  return (
    <aside className="flex h-full w-56 shrink-0 flex-col border-r border-panel-border bg-panel">
      {/* Logo */}
      <div className="flex items-center gap-2.5 border-b border-panel-border px-5 py-4">
        ...
        <div>
          <p className="text-sm font-semibold text-foreground">Vibe Score</p>
          <p className="text-[11px] text-muted">Coding Analytics</p>
        </div>
      </div>
          ...
          return (
            ...
            >
              ...
              <div className="min-w-0">
                ...
                <p className="mt-0.5 text-[11px] leading-none opacity-60">{item.hint}</p>
              </div>
            </Link>
          );
        })}
      ...
      <div className="border-t border-panel-border px-5 py-3">
        <p className="text-[11px] text-muted">macOS · local-first · tauri</p>
      </div>
    </aside>
  );
}
